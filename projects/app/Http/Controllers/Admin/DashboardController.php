...
class DashboardController extends Controller{
    ...
    function multipleDelete(Request $request,$model){
        foreach($request->id as $id){
            if($model=="galleries"){
                $data=\App\Models\Gallery::find($id);if($data){$data->delete();}
            }
            if($model=="testimonials"){
                $data=\App\Models\Testimonial::find($id);if($data){$data->delete();}
            }
            if($model=="blog-category"){
                $data=\App\Models\Blogs\BlogCategory::find($id);if($data){$data->delete();}
            }
            if($model=="blogs"){
                $data=\App\Models\Blogs\Blog::find($id);if($data){$data->delete();}
            }
            if($model=="faqs"){
                $data=\App\Models\Faq::find($id);if($data){$data->delete();}
            }
            if($model=="feedbacks"){
                $data=\App\Models\Feedback::find($id);if($data){$data->delete();}
            }
            if($model=="sliders"){
                $data=\App\Models\Slider::find($id);if($data){$data->delete();}
            }
            if($model=="newsletters"){
                $data=\App\Models\Newsletter::find($id);if($data){$data->delete();}
            }
        }
    }
    
   function index(){
        return redirect('admin/host_fully_properties');
