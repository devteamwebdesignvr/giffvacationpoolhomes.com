...
class LiveCart{
    ...
    function iCalDataCheckInCheckOut($property_id){
        $checkin=[];
        $checkout=[];
        $data=HostFullyPropertyPricePeriod::whereDate("from_date",">=",date('Y-m-d'))->where("isCheckinAllowed","0")->where("propertyUid",$property_id)->get();
        foreach($data as $c){
            $checkin[]=$c->from_date;
        }
        $data=HostFullyPropertyPricePeriod::whereDate("from_date",">=",date('Y-m-d'))->where("isCheckoutAllowed","0")->where("propertyUid",$property_id)->get();
        foreach($data as $c){
            $checkout[]=$c->from_date;
        }
        $data=HostFullyPropertyPricePeriod::whereDate("from_date",">=",date('Y-m-d'))->where("unavailable","1")->where("propertyUid",$property_id)->get();
        foreach($data as $c){
            $checkin[]=$c->from_date;
            $checkout[]=$c->from_date;
        }
    ...
    function allIcalImportListRefresh(){
        $data=HostFullyProperty::all();
        foreach($data as $ical_list){
            $this->refreshIcalData($ical_list->id,$ical_list->availabilityCalendarUrl,$ical_list->id,$ical_list->color);
        }
    }
